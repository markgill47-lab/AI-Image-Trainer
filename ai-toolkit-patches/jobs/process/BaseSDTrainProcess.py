    ...
    def process_general_training_batch(self, batch: 'DataLoaderBatchDTO'):
        with torch.no_grad():
            ...
            with self.timer('prepare_latents'):
                ...
                # DEBUG: Always print to see which path we take
                if not hasattr(self, '_debug_latent_path_count'):
                    self._debug_latent_path_count = 0
                if self._debug_latent_path_count < 3:
                    print(f"\n[LATENT PATH DEBUG] batch.latents is {'NOT None' if batch.latents is not None else 'None'}")
                    self._debug_latent_path_count += 1

                if batch.latents is not None:
                    latents = batch.latents.to(self.device_torch, dtype=dtype)

                    # DEBUG: Check latents when loaded from cache
                    if not hasattr(self, '_debug_latent_load'):
                        self._debug_latent_load = 0
                    if self._debug_latent_load < 5:  # Only print first 5 times
                        print(f"\n[LATENT LOAD DEBUG] Step {self._debug_latent_load}")
                        print(f"  Before dtype conversion: min={batch.latents.min().item():.4f}, max={batch.latents.max().item():.4f}, dtype={batch.latents.dtype}")
                        print(f"  After dtype conversion:  min={latents.min().item():.4f}, max={latents.max().item():.4f}, dtype={latents.dtype}")
                        if latents.abs().max() > 100:
                            print(f"  *** EXTREME VALUES DETECTED ***")
                            # Print file paths
                            for fi in batch.file_items:
                                print(f"    File: {fi.path}")
                        self._debug_latent_load += 1
