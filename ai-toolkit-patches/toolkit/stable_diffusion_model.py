...
class StableDiffusion:
    ...
    def encode_images(
        ...
        if not hasattr(self, '_debug_encode_once'):
            self._debug_encode_once = False
        if not self._debug_encode_once:
            print(f"\n=== ENCODE_IMAGES DEBUG ===")
            print(f"Input images dtype: {images.dtype}, device: {images.device}")
            print(f"Input images stats: min={images.min().item():.4f}, max={images.max().item():.4f}, mean={images.mean().item():.4f}")
            print(f"Input images has NaN: {torch.isnan(images).any()}, has Inf: {torch.isinf(images).any()}")
        ...
        # DEBUG: Check raw VAE output
        if not self._debug_encode_once:
            print(f"Raw VAE latents dtype: {latents.dtype}")
            print(f"Raw VAE latents stats: min={latents.min().item():.4f}, max={latents.max().item():.4f}, mean={latents.mean().item():.4f}")
            print(f"Raw VAE latents has NaN: {torch.isnan(latents).any()}, has Inf: {torch.isinf(latents).any()}")
        ...
        if not self._debug_encode_once:
            print(f"Scaling factor: {self.vae.config['scaling_factor']}, shift factor: {shift}")
            print(f"Scaled latents stats: min={latents.min().item():.4f}, max={latents.max().item():.4f}, mean={latents.mean().item():.4f}")
            print(f"Scaled latents has NaN: {torch.isnan(latents).any()}, has Inf: {torch.isinf(latents).any()}")

            # Check for extreme values
            if latents.abs().max() > 100:
                print(f"*** WARNING: Extreme latent values detected! Max abs: {latents.abs().max().item()}")
            self._debug_encode_once = True
            print(f"=== END ENCODE_IMAGES DEBUG ===\n")
