...
class AiToolkitDataset(LatentCachingMixin, ControlCachingMixin, CLIPCachingMixin, TextEmbeddingCachingMixin, BucketsMixin, CaptionMixin, Dataset):
    ...
    def __init__(
        ...
        # DEBUG: Print cache settings
        print(f"\n=== DATASET CONFIG DEBUG ===")
        print(f"  cache_latents from config: {dataset_config.cache_latents}")
        print(f"  is_caching_latents: {self.is_caching_latents}")
        print(f"  is_caching_latents_to_memory: {self.is_caching_latents_to_memory}")
