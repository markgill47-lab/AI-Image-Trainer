...
class SDTrainer(BaseSDTrainProcess):
    ...
    def __init__(self, process_id: int, job, config: OrderedDict, **kwargs):
        ...
        # DEBUG flag for loss calculation
        self._debug_first_loss = False
    ...
    def hook_before_train_loop(self):
        ...
        # Skip caching for models that require dynamic embeddings (e.g., FLUX Klein)
        requires_dynamic_embeddings = getattr(self.sd, 'requires_dynamic_text_embeddings', False)
        if (self.train_config.unload_text_encoder or self.is_caching_text_embeddings) and not requires_dynamic_embeddings:
    ...
    def calculate_loss(
        ...
        if self.train_config.correct_pred_norm or (self.train_config.inverted_mask_prior and prior_pred is not None and has_mask):
            ...
            if not self._debug_target_calc:
                print(f"\n=== TARGET CALCULATION (flow matching) ===")
                print(f"noise stats: min={noise.min().item():.4f}, max={noise.max().item():.4f}, mean={noise.mean().item():.4f}")
                print(f"batch.latents stats: min={batch.latents.min().item():.4f}, max={batch.latents.max().item():.4f}, mean={batch.latents.mean().item():.4f}")
        ...
        if loss_target == 'source' or loss_target == 'unaugmented':
            ...
            if self.train_config.loss_type == "mae":
                ...
                # DEBUG: Check pred and target before loss calculation
                if hasattr(self, '_debug_first_loss') and not self._debug_first_loss:
                    print(f"\n=== FIRST LOSS CALCULATION ===")
                    print(f"pred shape: {pred.shape}, target shape: {target.shape}")
                    print(f"pred stats: min={pred.min().item():.4f}, max={pred.max().item():.4f}, mean={pred.mean().item():.4f}")
                    print(f"target stats: min={target.min().item():.4f}, max={target.max().item():.4f}, mean={target.mean().item():.4f}")
                    print(f"pred has NaN: {torch.isnan(pred).any()}, has Inf: {torch.isinf(pred).any()}")
                    print(f"target has NaN: {torch.isnan(target).any()}, has Inf: {torch.isinf(target).any()}")
    ...
    def hook_train_loop(self, batch: Union[DataLoaderBatchDTO, List[DataLoaderBatchDTO]]):
        # DEBUG: Print at start of every train loop to confirm code is running
        if not hasattr(self, '_hook_train_debug_count'):
            self._hook_train_debug_count = 0
        if self._hook_train_debug_count < 3:
            print(f"\n=== HOOK_TRAIN_LOOP DEBUG #{self._hook_train_debug_count} ===")
            self._hook_train_debug_count += 1
