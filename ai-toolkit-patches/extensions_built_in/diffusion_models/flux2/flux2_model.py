...
class Flux2Model(BaseModel):
    ...
    def get_noise_prediction(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            # Ensure text_embeds has the right shape for batched processing
            text_embeds = text_embeddings.text_embeds
            if text_embeds.dim() == 2:
                # Add batch dimension if missing
                text_embeds = text_embeds.unsqueeze(0)

            txt, txt_ids = batched_prc_txt(text_embeds)
            packed_latents, img_ids = batched_prc_img(latent_model_input)

            # For Klein models: truncate text to match image sequence length
            # Klein uses variable-length text embeddings that must match image dimensions
            if hasattr(self, 'requires_dynamic_text_embeddings') and self.requires_dynamic_text_embeddings:
                img_seq_len = img_ids.shape[1]
                if txt.shape[1] > img_seq_len:
                    txt = txt[:, :img_seq_len, :]
                    txt_ids = txt_ids[:, :img_seq_len, :]
    ...
    def get_loss_target(self, *args, **kwargs):
        ...
        if not hasattr(self, '_debug_target_calc'):
            self._debug_target_calc = False
        if not self._debug_target_calc:
            print(f"\n=== TARGET CALCULATION (get_loss_target) ===")
            print(f"noise stats: min={noise.min().item():.4f}, max={noise.max().item():.4f}, mean={noise.mean().item():.4f}, std={noise.std().item():.4f}")
            print(f"batch.latents stats: min={batch.latents.min().item():.4f}, max={batch.latents.max().item():.4f}, mean={batch.latents.mean().item():.4f}, std={batch.latents.std().item():.4f}")
            print(f"noise has NaN: {torch.isnan(noise).any()}, has Inf: {torch.isinf(noise).any()}")
            print(f"batch.latents has NaN: {torch.isnan(batch.latents).any()}, has Inf: {torch.isinf(batch.latents).any()}")
            self._debug_target_calc = True
    ...
    def encode_images(self, image_list: List[torch.Tensor], device=None, dtype=None):
        ...
        # DEBUG: Check VAE encoding
        if not hasattr(self, '_debug_encode_count'):
            self._debug_encode_count = 0
        if self._debug_encode_count < 3:
            print(f"\n[VAE ENCODE DEBUG] Call #{self._debug_encode_count}")
            print(f"  Input images: shape={images.shape}, dtype={images.dtype}, min={images.min().item():.4f}, max={images.max().item():.4f}")
            # Check BatchNorm stats
            if hasattr(self.vae, 'bn'):
                bn = self.vae.bn
                print(f"  VAE BatchNorm running_mean: min={bn.running_mean.min().item():.4f}, max={bn.running_mean.max().item():.4f}")
                print(f"  VAE BatchNorm running_var: min={bn.running_var.min().item():.4f}, max={bn.running_var.max().item():.4f}")

        latents = self.vae.encode(images)

        if self._debug_encode_count < 3:
            print(f"  Output latents: shape={latents.shape}, dtype={latents.dtype}, min={latents.min().item():.4f}, max={latents.max().item():.4f}")
            if latents.abs().max() > 100:
                print(f"  *** WARNING: Extreme latent values detected! ***")
            self._debug_encode_count += 1
