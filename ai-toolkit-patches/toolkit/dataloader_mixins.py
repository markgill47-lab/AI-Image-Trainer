            ...
            for file_item in tqdm(self.file_list, desc=f'Caching latents{" to disk" if to_disk else ""}'):
                ...
                if os.path.exists(latent_path):
                    ...
                    try:
                        ...
                        # DEBUG: Check image tensor before encoding
                        print(f"[CACHE] Encoding: {file_item.path}")
                        print(f"[CACHE] Image tensor: dtype={imgs.dtype}, min={imgs.min().item():.4f}, max={imgs.max().item():.4f}")

                        latent = self.sd.encode_images(imgs).squeeze(0)

                        # DEBUG: Check latent right after encoding
                        print(f"[CACHE] Latent after encode: dtype={latent.dtype}, min={latent.min().item():.4f}, max={latent.max().item():.4f}")
                        if latent.abs().max() > 100:
                            print(f"*** WARNING: Extreme latent detected right after encode! ***")
                    ...
                    if to_memory:
                        ...
                        # DEBUG: Check latent right after encoding
                        if file_item._encoded_latent.abs().max() > 100:
                            print(f"\n*** LATENT CORRUPTED DURING CACHING ***")
                            print(f"File: {file_item.path}")
                            print(f"Latent stats: min={file_item._encoded_latent.min().item():.4f}, max={file_item._encoded_latent.max().item():.4f}")
                            print(f"Latent dtype: {file_item._encoded_latent.dtype}")
                            print(f"Original latent (before to cpu): min={latent.min().item():.4f}, max={latent.max().item():.4f}, dtype={latent.dtype}")
