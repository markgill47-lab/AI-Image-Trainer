...
def compute_empirical_mu(image_seq_len: int, num_steps: int) -> float:
    """
    Compute mu for schedule shifting using BFL's empirical formula.
    This is the official formula from black-forest-labs/flux2.
    """
...
def get_schedule(
    ...
    shift: bool = True,
    use_bfl_schedule: bool = True,  # Use BFL's empirical schedule by default
) -> list[float]:
    # extra step for zero
    ...
    if shift:
        if use_bfl_schedule:
            # Use BFL's empirical formula (official from black-forest-labs/flux2)
            mu = compute_empirical_mu(image_seq_len, num_steps)
        else:
            # Legacy: estimate mu based on linear estimation between two points
