        ...
        try:
            ...
            if is_latents_cached:
                ...
                for idx, x in enumerate(self.file_items):
                    lat = x.get_latent()
                    # DEBUG: Check each latent as it's loaded
                    if lat.abs().max() > 100:
                        print(f"\n*** CORRUPT LATENT DETECTED ***")
                        print(f"File: {x.path}")
                        print(f"Latent stats: min={lat.min().item():.4f}, max={lat.max().item():.4f}")
                        print(f"Latent dtype: {lat.dtype}")
                    latent_list.append(lat.unsqueeze(0))
