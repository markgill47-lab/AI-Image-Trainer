...
class LoRASpecialNetwork(ToolkitNetworkMixin, LoRANetwork):
    ...
    def __init__(
        ...
        if is_flux:
            # Check if base_model specified custom target modules
            if base_model is not None and hasattr(base_model, 'target_lora_modules'):
                target_modules = base_model.target_lora_modules
            elif target_lin_modules == LoRANetwork.UNET_TARGET_REPLACE_MODULE:
                # Only use default if custom target modules weren't provided
